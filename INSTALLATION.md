...
Seed создаст тестовые данные:
- **6 пользователей** (1 админ, 2 клиента, 2 водителя, 1 продавец)
- **2 профиля водителей** (1 верифицированный, 1 неверифицированный)
- **1 профиль продавца** (верифицированный)
- **2 магазина** с товарами
- **5 товаров** (3 одобренных, 2 неодобренных)
- **3 заказа услуг** с разными статусами
- **2 заказа в маркетплейсе**

#### Тестовые учетные записи:

| Роль | Telegram ID | Имя | Описание |
|------|-------------|-----|----------|
| Админ | `111111111` | Администратор | Полный доступ к админ-панели |
| Клиент | `222222222` | Иван Петров | Тестовый клиент с активным заказом |
| Клиент | `333333333` | Мария Сидорова | Тестовый клиент |
| Водитель | `444444444` | Алексей Водителев | Верифицированный водитель (4.8⭐) |
| Водитель | `555555555` | Сергей Автосервис | Неверифицированный водитель |
| Продавец | `666666666` | Магазин АвтоПлюс | Верифицированный продавец |
...
Админ-панель доступна по адресу `/admin` и требует прав администратора.
...
После seed команды у вас уже есть админ с `telegramId = 111111111`. Для назначения другого пользователя администратором:
...
2. **Через SQL:**
```bash
psql yakgo -c "UPDATE \"User\" SET \"isAdmin\" = true WHERE \"telegramId\" = 'YOUR_TELEGRAM_ID';"
```

### Функции админ-панели

- 📊 **Дашборд со статистикой:**
  - Общее количество пользователей, водителей, продавцов
  - Количество заказов и продуктов
  - Количество ожидающих верификации

- 🚗 **Управление водителями:**
  - Просмотр списка всех водителей
  - Верификация/отмена верификации водителей
  - Просмотр рейтинга и статистики

- 🏪 **Управление продавцами:**
  - Просмотр списка всех продавцов
  - Верификация/отмена верификации продавцов
  - Просмотр информации о магазинах

- 📦 **Управление товарами:**
  - Просмотр списка всех товаров
  - Одобрение/отклонение новых товаров
...
// Тест отслеживания водителя
socket.emit('order:subscribe', 'SAHA-0001');
socket.on('driver:location:update', (data) => {
  console.log('Driver location:', data);
});
```

### Интерактивная карта с отслеживанием:

1. **Для клиента:**
   - Откройте дашборд при активном заказе
   - Вы увидите карту с двумя маркерами:
     - 📍 Синий маркер - ваше местоположение (статичное)
     - 🚗 Зеленый маркер - местоположение водителя (обновляется в реальном времени)
   - Карта автоматически подписывается на обновления через WebSocket

2. **Для водителя:**
   - Примите заказ в панели водителя
   - Браузер запросит доступ к геолокации - разрешите
   - Ваше местоположение будет автоматически отправляться каждые 5 секунд
   - Клиент увидит ваше движение на карте в реальном времени
