...
const simulateAiDiagnosis = (description: string): DiagnoseProblemOutput => {
  ...
  const fuelKeywords =
    /\b(топлив|бензин|солярк|пустой бак|кончился бензин|заглох на трассе)\b/;
  ...
  const assistanceKeywords =
    /\b(аккумулятор|акб|сел|разрядился|не крутит стартер|прикуриван|колес[ао]|прокол|спустило|порезана шина|вулканизаци|заглох|не заводится|троит двигатель|ключ[ией]|захлопнулась дверь|вскрытие|перегрев|закипел|зажигание|щелкает|крутит|сломалась|не едет|генератор|лампа акб|ремень генератора|свист|течет масло|течет антифриз|пятно под машин|фильтр|дергается|стартер|check engine|электроник|патрубк|шланг|турбин|дым|свечи|сигнализац|гур|тяжелый руль|стекло|лобовое|давления масла|предохранитель|печка|сажевый фильтр|выхлоп|сцепление|инжектор|катализатор|шрус|тормозной шланг|давления масла|сигнализац|насос гур)\b/;
